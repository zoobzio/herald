...
```go
func New(topicOrSub string, opts ...Option) *Provider
```
...
```go
func NewPostgresAdapter(db *sql.DB) DB
func NewMySQLAdapter(db *sql.DB) DB
func NewSQLiteAdapter(db *sql.DB) DB
```
