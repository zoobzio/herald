...
```go
import "github.com/zoobzio/herald/pkg/sql"
```
...
```go
func NewPostgres(db *sql.DB, opts ...PostgresOption) (DB, error)
func NewMySQL(db *sql.DB, opts ...MySQLOption) (DB, error)
func NewSQLite(db *sql.DB, opts ...SQLiteOption) (DB, error)
```

Each adapter creates the messages table if it doesn't exist.
