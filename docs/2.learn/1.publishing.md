...
3. **Extract and serialize** — The publisher extracts the typed value using the key, then marshals it to bytes using the codec (JSON by default).

4. **Attach metadata** — Context metadata is extracted and attached to the message.

5. **Publish to broker** — The serialized message and metadata are sent to the broker via the provider.
...
Metadata attached to the context flows through to the broker:

```go
// Attach metadata before emitting
